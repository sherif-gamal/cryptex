...
const addTodo = async (driver, key) => {
  ...
  await delay(1000);
  const watchlist = await findList(driver);
  return { todo: watchlist[0], count: watchlist.length };
};

const editTodo = async (driver, index, key) => {
  let watchlist = await findList(driver);
  const label = watchlist[index].findElement(webdriver.By.tagName('label'));
  ...
  watchlist = await findList(driver);
  return { todo: watchlist[index], count: watchlist.length };
};

const completeTodo = async (driver, index) => {
  let watchlist = await findList(driver);
  watchlist[index].findElement(webdriver.By.className(todoItemStyle.toggle)).click();
  await delay(1000);
  watchlist = await findList(driver);
  return { todo: watchlist[index], count: watchlist.length };
};
...
const deleteTodo = async (driver, index) => {
  ...
  watchlist[index].findElement(webdriver.By.className(todoItemStyle.destroy)).click();
  await delay(1000);
  watchlist = await findList(driver);
  return { count: watchlist.length };
};
...
describe('window (popup) page', function test() {
  ...
  it('should can complete all watchlist', async () => {
    driver.findElement(webdriver.By.className(mainSectionStyle.toggleAll)).click();
    const watchlist = await findList(driver);
    const classNames = await Promise.all(watchlist.map(todo => todo.getAttribute('class')));
    const { completed, normal } = todoItemStyle;
  ...
  it('should can clear completed watchlist if completed watchlist count > 0', async () => {
  ...
  it('should cannot clear completed watchlist if completed watchlist count = 0', async () => {
    const watchlist = await driver.findElements(webdriver.By.className(footerStyle.clearCompleted));
    expect(watchlist.length).to.equal(0);
  });

  it('should can filter active watchlist', async () => {
    ...
    await completeTodo(driver, 0);
    let watchlist = await driver.findElements(webdriver.By.css(`.${footerStyle.filters} > li`));
    watchlist[1].click();
    await delay(1000);
    watchlist = await findList(driver);
  ...
  it('should can filter completed watchlist', async () => {
    // current todo count: 2
    let watchlist = await driver.findElements(webdriver.By.css(`.${footerStyle.filters} > li`));
    watchlist[2].click();
    await delay(1000);
